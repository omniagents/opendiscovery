...
curl -s -X POST -H "Content-Type: application/json" -d '{"network_id": "network-12345678", "num_agents": 3}' http://localhost:5000/apis/heartbeat | python -m json.tool
...
curl -s -X POST -H "Content-Type: application/json" -d '{"network_id": "network-12345678"}' http://localhost:5000/apis/unpublish | python -m json.tool
